...
class UserModel extends Model
{
    ...
    public function getUsers($allUsers=false, $status='')
    {
		try {
			$users = 
			$this->db->table('_users')->select('_users.id, _users.tenantid, _users.email, rolename, firstname, lastname, CONCAT(_files.path, _files.name) AS profileimage,
			_users.lastinat, _users.status')
    ...
    public function getUser($id=0)
    {
		try {
			$users = 
			$this->db->table('_users')->select('_users.id, _users.tenantid, email, roleid, rolename, firstname, lastname, dob, gender, CONCAT(_files.path, _files.name) AS profileimage,
			description, mobile, address1, address2, city, country, lastinat, _users.status')
			->join('_roles', '_roles.id = _users.roleid')
			->join('_files', '_files.id = _users.profileimageid', 'left')
			->where('_users.tenantid', 1)
			->where('_users.id', $id)
	...
	public function getTrainers($except=0) {
		try {
			...
			$trainers = 
			$this->db->table('_users')->select('_users.id, rolename, firstname, lastname, email, CONCAT(_files.path, _files.name) AS profileimage, description')
			->join('_roles', '_roles.id = _users.roleid')
			->join('_files', '_files.id = _users.profileimageid', 'left')
			->where('_users.tenantid', 1)
